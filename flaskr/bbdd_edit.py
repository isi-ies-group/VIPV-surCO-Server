...
from flaskr.models import DatoPersona, DatoSensor, User, SensorAUT


# 976
def EliminarDato(Bbdd, atributo, valor):
    datos_a_borrar = (
        db.session.query(Bbdd).filter(getattr(Bbdd, atributo).like(valor)).all()
    )
    if datos_a_borrar:
        ...
        print(
            "No se encontró ningún dato que coincida con el atributo y valor especificados"
        )
