...
from flaskr.models import DatoPersona, DatoSensor, SensorAUT, User
...
def AsignarPuntos(usuario):
    ...
    for entrada in (
        db.session.query(DatoPersona).filter_by(username=usuario.email).all()
    ):
        puntosMan = puntosMan + entrada.analisis

    puntosSensores = 0
    for entrada in (
        db.session.query(SensorAUT).filter_by(username_asociado=usuario.username).all()
    ):
        puntosSensores = (
            puntosSensores
            + db.session.query(DatoSensor)
            .filter_by(id_sensor=entrada.id_sensor)
            .count()
        )
