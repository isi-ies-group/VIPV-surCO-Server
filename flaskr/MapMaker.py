from flaskr.models import DatoPersona, DatoSensor, TipoMedidaEnum, SensorAUT, User
import flaskr.db as db
from flaskr.Score import calcular_puntuacion_entrada
...
def mapear(long, latt, valor, texto, imagen_path, capa):
    ...
    marcador = folium.Marker(
        location=[long, latt], popup=popup, icon=folium.Icon(color=color)
    )
...
def MakeMap():
    ...
    for entrada in db.session.query(DatoPersona).all():
        ...
        mapear(
            entrada.latitud,
            entrada.longitud,
            calcular_puntuacion_entrada(entrada, DatoPersona),
            cadena_str,
            entrada.url,
            capa_Epicollect,
        )
        ...
        )  # Añadir los valores a la lista como una tupla
    ...
    for entrada in db.session.query(DatoSensor).all():
        ...
        if entrada.tipo_medida == TipoMedidaEnum("fotografia"):
            ...
            mapear(
                ...
                entrada.timestamp,
                entrada.valor,
                capa_Imagenes_sensores,
            )
        DatosSensores.append(
            (entrada.latitud, entrada.longitud)
        )  # Añadir los valores a la lista como una tupl
    ...
    madrid_map.save("instance/madrid_map.html")
...
def MakeUserMap(usuario):
    print(TipoMedidaEnum)
    print("Creando Mapa...")
    if os.path.exists("instance/usermap.html"):
        os.remove("instance/usermap.html")
    ...
    DatosSensores = []
    for entrada in (
        db.session.query(SensorAUT).filter_by(username_asociado=usuario).all()
    ):
        for sensor in (
            db.session.query(DatoSensor).filter_by(id_sensor=entrada.id_sensor).all()
        ):
